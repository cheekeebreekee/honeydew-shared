...
const newAppointment = async (careCoordinator, patient) => {
    const { firstName, lastName, phone } = careCoordinator;
    (0, logger_1.logInfo)("Sending SMS message to care coordinator about new appointment", {
        careCoordinator,
        patient,
    });
    const smsMessageParams = {
        Message: `Dear ${firstName} ${lastName}.\nPatient ${(0, get_patient_initials_1.getPatientInitials)(patient)} (DOB ${patient.basicInfo.birthdate}) has a new appointment at ${(0, get_date_pretty_1.getDatePretty)(patient.appointments[0].start_time)}. \nPatient's phone number: ${patient.phone}`,
